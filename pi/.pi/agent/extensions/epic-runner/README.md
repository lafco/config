...
```
1. list_story_tasks        → tarefas da onda N
...
5. update_task_status      → pronto/bloqueado
6. (próxima onda)          → volta ao passo 1
7. remove_task_worktrees   → limpeza
```
...
| `tasks.ts` | Parse do frontmatter/índice, `selectReadyWave`, `detectFileConflicts`, escrita de status/evidência |

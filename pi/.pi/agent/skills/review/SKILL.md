...
For a change since a fixed point (branch, PR, commit, tag, merge-base) reviewed along the Standards and Spec axes, use the `code-review` skill instead: it dispatches both axes in parallel and reports them side by side. This skill is for reviewing an artifact on its own terms.

---
name: explainer
description: Produces a rich, self-contained HTML explainer (background, intuition, rationale, code walkthrough, interactive quiz) for an implemented change
...
Registre no topo do documento um bloco de **Procedência**: o alvo (ref/range) e a solicitação de origem.
...
### 3. Why this way
A seção que o código bruto não dá: o que o diff não conta. Para cada decisão de projeto relevante:
...
Isto é o diff "literate": um diff cru é matéria-prima, isto é a explicação dele.
